This is the patched version of Cosmjs' Launchpad package, adapted
to work properly with the ixo blockchain which is currently built
with the Cosmos SDK 0.37. Although the Launchpad package is
apparently designed to have support for Cosmos SDK 0.37 as well,
for some reason there are several incompatibilities that can't
easily be handled by extending the package nor monkey patching.
The "patch-package" package was considered to deal with this
situation, but it doesn't support to be used in a library (it
requires a top level project). Then forking Launchpad was
considered, but since the whole CosmJS tools are maintained in a
monorepo (managed with Lerna) and it was written in Typescript, it
was extremely challenging to adapt the simple patches made to the
build to the fork's source code.

Since this is a temporary requirement (as the ixo blockchain will
migrate to the Cosmos SDK 0.38 very soon, and then to 0.4), we no
longer want to bother with "better" nor "cleaner" solutions and
waste time but instead just to put the patched build here for the
ixo client SDK to depend on temporarily.

For reference, the patch is in the root directory as well so you
can examine the changes made to the original CosmJS Launcpad
package.

Thanks for reading, continue with the original README below:
